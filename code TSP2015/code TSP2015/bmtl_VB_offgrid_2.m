function [X Beta]=bmtl_VB_offgrid_2(A,B,Y,resolution,hyper)
...
%     hyper.d = ones(N,hyper.truncateL)*1e-6;
    ...
    hyper.alpha0 =  (1/std(Y(:)))^2*1e3;%1/ 1e-1;hyper.a / hyper.b 
...
for ii = 1 : M;
    ...
    temp_mu_x = zeros(N,1);
    for jj= 1 : 1
        Phi_k = A((ii-1)*nd+1:ii*nd,:)+B((ii-1)*nd+1:ii*nd,:)*diag(mu_beta);
        PhikHPhik = Phi_k'*Phi_k;
        temp_sigma_x = temp_sigma_x + (diag(alpha(:,jj))+alpha0*(PhikHPhik));
        temp_mu_x = temp_mu_x +  Phi_k'* Y( : , ii );
    end
...
%     end
    ...
    for ii= 1 : M
        ...
        temp_p_beta = temp_p_beta + (conj(BHB).*(mu_x(idx,ii)*mu_x(idx,ii)'+Sigma_x(idx,idx,ii)));
        temp_v_beta = temp_v_beta + (conj(mu_x(idx,ii)).*(B((ii-1)*nd+1:ii*nd,idx)'*(Y( : , ii )-A((ii-1)*nd+1:ii*nd,:)*mu_x(:,ii)))-diag(BHA*Sigma_x(:,idx,ii)));
    end
...
%             + 2*real(Y( : , ii )'*B((ii-1)*nd+1:ii*nd,:)*diag(mu_beta(:,jj))*mu_x(:,ii)-trace(AHB*diag(mu_beta(:,jj))*(ita_2)));
        ita(jj,ii) = norm(Y( : , ii )-(A((ii-1)*nd+1:ii*nd,:)+B((ii-1)*nd+1:ii*nd,:)*diag(mu_beta))*mu_x(:,ii),'fro')^2 ...
        + trace((A((ii-1)*nd+1:ii*nd,:)+B((ii-1)*nd+1:ii*nd,:)*diag(mu_beta))'*(A((ii-1)*nd+1:ii*nd,:)+B((ii-1)*nd+1:ii*nd,:)*diag(mu_beta))*Sigma_x(:,:,ii));
%         if jj == 1;
%         temp1(jj,ii) = 0;
...
%  for ii=1:M
%     pos=find(2.5>alpha(:,ii) & alpha(:,ii)>0.01);
%     alpha(pos,ii)=100;
%  end
for ii = 1 : M;
    temp_sigma_x = zeros(N,N);
