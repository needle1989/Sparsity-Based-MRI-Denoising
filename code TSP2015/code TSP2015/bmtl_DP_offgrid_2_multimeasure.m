function [X Beta]=bmtl_DP_offgrid_2_multimeasure(A,B,Y,resolution,hyper)
...
%     hyper.pi = zeros(hyper.truncateL,1);
    ...
    for ii = 1:M
        ...
        hyper.alpha(:,ii) =  (1 ./ abs(A((ii-1)*nd+1:ii*nd,:)' * Y{1}(:,ii)));%ones(N,1)*inf
...
%         temp3(jj,ii) = 0;
        ...
        temp4(jj,ii) = N_measure*real( sum( log((cnew(:,jj)+eps) ) - log((dnew(:,jj)) ) )) + temp4(jj,ii);
