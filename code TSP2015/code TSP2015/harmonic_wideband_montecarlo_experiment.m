...
snr =[-44.7 -42.3 -39.7 -37.2 -34.8 -32.2 -29.8];
SNR = zeros(length(snr),1);
Err_1=zeros(length(snr),K);
Err_2=zeros(length(snr),K);
Err_3=zeros(length(snr),K);
%N_exp = 10;
N_exp = 50;%50;
...
SNR_e=[-7.5 -5 -2.5 0 2.5 5 7.5];
err1=sum(Err_1,2)/K;
err2=sum(Err_2,2)/K;
err3=sum(Err_3,2)/K;
figure
plot(SNR_e,err1,'r-o',SNR_e,err1,'r-*',SNR_e,err1,'r-^');
xlabel('SNR(dB)');ylabel('RMSE(Deg.)')
axis tight;
N_subband=length(pos_channel);
%save Err_31.mat Err_1 Err_2 Err_3 N_subband;
