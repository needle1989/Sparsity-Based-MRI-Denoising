function est_theta_DP_S_1=DPSeperatedSearch_multimeasure(Xdp_hat,alpha_DP,theta_ex,alpha0_DP,pos_channel,Y_temp,A_temp);
i=sqrt(-1);
N_measurements=length(Y_temp);
T=256;
M=10;
C=1500;
d=1.5;
...
%  nn=11;
 ...
 R_y_f = zeros(M,M);
 for nnn=1:N_measurements
 R_y_f = R_y_f+Y_temp{nnn}(:,pos_test_temp(nn))*Y_temp{nnn}(:,pos_test_temp(nn))';
 end
 R_y_f = R_y_f/N_measurements;
...
% hold on;plot(theta(pos_temp), 10*log10([1].^2), 'bo');
