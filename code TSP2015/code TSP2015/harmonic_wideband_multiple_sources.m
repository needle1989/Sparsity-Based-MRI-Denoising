...
theta = [58.71 108.42 79.65 131.2]';%
...
snr = -44.7;
N_exp = 5;
Err_DP_I=zeros(K,(N_exp));
Err_DP_S=zeros(K,(N_exp));
Err_RVM_S=zeros(K,(N_exp));
...
temp_a=temp(72);
pos_channel = find(max(abs(Xbeam_hat(:,5:end)))>temp_a)+4;
...
% xlabel('frequency channels (Hz)')
% ylabel('direction')
[ind alpha_DP]=kmeans(alpha_m.',K,'EmptyAction','drop');
theta_ex = [58 60];
...
theta_ex = [78 80];
est_theta_DP_S_3=DPSeperatedSearch(Xdp_hat,alpha_DP,theta_ex,alpha0_DP,pos_channel,Y_temp,A_temp,theta(3));
theta_ex = [130 132];
est_theta_DP_S_4=DPSeperatedSearch(Xdp_hat,alpha_DP,theta_ex,alpha0_DP,pos_channel,Y_temp,A_temp,theta(4));
Err_DP_S(1,mmm) = (abs(est_theta_DP_S_1-theta(1)));
Err_DP_S(2,mmm) = (abs(est_theta_DP_S_2-theta(2)));
Err_DP_S(3,mmm) = (abs(est_theta_DP_S_3-theta(3)));
Err_DP_S(4,mmm) = (abs(est_theta_DP_S_4-theta(4)));
% 
% % %% ------------------------RVM zhangmeng Liu---------------------
...
theta_ex = [76 78 80 82];
est_theta_RVM_3=RVMSeperatedSearch(theta_ex,alpha,alpha0,pos_channel,Y_temp,A_temp,theta(3));
theta_ex = [128 130 132 134];
est_theta_RVM_4=RVMSeperatedSearch(theta_ex,alpha,alpha0,pos_channel,Y_temp,A_temp,theta(4));
Err_RVM_S(1,mmm) = (abs(est_theta_RVM_1-theta(1)));
Err_RVM_S(2,mmm) = (abs(est_theta_RVM_2-theta(2)));
Err_RVM_S(3,mmm) = (abs(est_theta_RVM_3-theta(3)));
Err_RVM_S(4,mmm) = (abs(est_theta_RVM_4-theta(4)));
%% -----------------------MTL-CS-DP-(off-grid)---------------------------%% 
Xdp_offgrid_hat_h=zeros(N,C_N);
[Xdp_offgrid_hat X_Beta]=bmtl_DP_offgrid_2(A_temp,B_temp,Y_temp,K,resolution);
...
if min(abs(est_theta-theta(1)))>2
  ...
  Err_DP_I(1,mmm) = min(abs(est_theta-theta(1)));  
end
if min(abs(est_theta-theta(2)))>2
   Err_DP_I(2,mmm) = 2;
else
  Err_DP_I(2,mmm) = min(abs(est_theta-theta(2)));  
end
if min(abs(est_theta-theta(3)))>2
   Err_DP_I(3,mmm) = 2;
else
  Err_DP_I(3,mmm) = min(abs(est_theta-theta(3)));  
end
if min(abs(est_theta-theta(4)))>2
   Err_DP_I(4,mmm) = 2;
else
  Err_DP_I(4,mmm) = min(abs(est_theta-theta(4)));  
end
%% ------------------------CSSM-------------------------------------------
...
% Err_CSSM(2,nnn) = Err_CSSM(2,nnn)/N_exp;
 ...
 Err_1=sum(Err_RVM_S,2)/N_exp;
 Err_1_m=mean(Err_1);
 Err_2=sum(Err_DP_S,2)/N_exp;
 Err_2_m=mean(Err_2);
 Err_3=sum(Err_DP_I,2)/N_exp;
 Err_3_m=mean(Err_3);
